...
export function TeacherStudentTabs({ studentId }: TeacherStudentTabsProps) {
  ...
  return (
    <div className="ui-tab-shell rounded-[12px] p-1 sm:rounded-[10px] sm:p-1.5">
      <nav className="ui-tab-strip flex min-w-0 gap-1 overflow-x-auto sm:gap-1.5" aria-label="Разделы ученика">
        {items.map((item) => (
          <Link
            ...
            className="ui-pressable ui-tab shrink-0 rounded-[10px] px-3 py-2 text-[0.8rem] font-medium sm:rounded-[12px] sm:px-4 sm:py-2.5 sm:text-sm"
          >
            {item.label}
          </Link>
        ))}
