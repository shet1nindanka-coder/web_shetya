...
Объём задаётся бюджетом budgetMinutes — это минуты домашней работы в день, заданные учителем, умноженные на daysUntilDeadline (дней до следующего занятия). Считать объём по сроку самостоятельно не нужно: бюджет уже посчитан. Также тебе дано досье ученика: скорость по шкале 1–10 (1 — очень медленный, 5–6 — средний темп, 10 — очень быстрый), заметка учителя, последние ошибки автопроверки (recentMistakes: тема, номер, тип ошибки errorKind и разбор note) и свёртка по темам (errorProfile) — это вердикты ИИ, сигнал, а не истина, активность, карта тем и история выданных ДЗ (homeworkHistory: тема, дата, дедлайн и итог — сколько решено верно/с ошибками/не тронуто) и история занятий (lessonHistory: дата, темы и итог урока — solved решил, partial решил с ошибками, notSolved не решил, skipped не успел (до номера на уроке не дошли), unmarked без отметки). История нужна для понимания, что и когда выдавалось; объём по ней НЕ снижай: бюджет должен быть заполнен, даже если прошлые ДЗ решены не до конца — от ученика требуется больше времени дома, а не меньше задач.
  ...
  try {
    ...
    if (available.length > shortlistSize) {
      const shortlistUser = JSON.stringify({
        homework: {
          daysUntilDeadline,
          budgetMinutes: params.dailyMinutes * daysUntilDeadline,
