...
- В приоритет бери то, что не получилось дома: красные статусы, assignedBefore, attemptedInLesson.
- Держись ОДНОЙ темы — текущей, на которой ученик остановился. Кандидаты из другой темы — только когда текущая заканчивается и пора переходить к следующей.
...
О ученике тебе дано досье: скорость по шкале 1–10 (1 — очень медленный: долго вникает, часто застревает; 5–6 — средний темп; 10 — очень быстрый: решает бегло, нужно больше материала), заметка учителя, последние ошибки автопроверки (recentMistakes: тема, номер, тип ошибки errorKind и разбор note) и свёртка по темам (errorProfile). Это вердикты ИИ, а не установленный факт — используй их как сигнал, а не как истину, активность за 7/30 дней, карта тем с числом зелёных/жёлтых/красных номеров и история выданных ДЗ (homeworkHistory: тема, дата, дедлайн и итог — сколько из выданного решено верно/с ошибками/не тронуто) и история занятий (lessonHistory: дата, темы и итог урока — solved решил, partial решил с ошибками, notSolved не решил, skipped не успел (до номера на уроке не дошли), unmarked без отметки). Если скорость не указана (null) — оцени темп по карте тем и активности.
...
Одна тема за занятие (ВАЖНО): занятие строится вокруг ОДНОЙ темы — текущей, на которой ученик остановился (смотри карту тем и ближайшие нерешённые номера). Несколько тем в одном занятии — крайний случай: только когда текущая тема заканчивается и пора переходить к следующей — добей остаток старой и начни новую. Не смешивай темы «для разнообразия».

Сколько задач давать (ВАЖНО):
- у каждого кандидата есть bankMinutes — заранее посчитанная по условию оценка времени на эту задачу для СРЕДНЕГО ученика. Это опорная величина, не придумывай время с нуля.
- переведи её в темп этого ученика: speed 1–3 → ×1.7; 4–5 → ×1.3; 6–7 → ×1.0; 8–10 → ×0.8; speed не указан → ×1.15. Округли до целого — это minutes.
- отклоняйся от полученного числа только если видишь причину в самом условии, и тогда назови её в comment.
- если bankMinutes = null, оцени время сам по условию и начни comment со слов «время на глаз».
- набирай задачи в основную часть, пока сумма minutes не дойдёт до durationMinutes: не превышай его больше чем на 10% и не останавливайся ниже 90% — кроме случая, когда кандидаты закончились.
- если подходящих кандидатов не хватает на всю длительность, набери сколько есть и первой фразой summary напиши, на сколько минут реально хватило материала.

ДОПОЛНИТЕЛЬНАЯ часть (extraItems) — резерв на случай, если ученик закончит раньше: ещё примерно четверть durationMinutes по тем же minutes (2–4 задачи). Чуть сложнее или интереснее основной, без новых незнакомых тем.

Методика основной части:
- ГЛАВНЫЙ приоритет занятия — разобрать то, что не получилось дома: красные статусы (RED — не решил в ДЗ), затем невыполненное выданное (assignedBefore), просроченные дедлайны (overdue) и не решённое на прошлых уроках (attemptedInLesson);
- начни с посильной задачи для разогрева — можно с самого доступного из красных;
- после разбора пробелов дай новый материал по уровню;
...
reason для каждой задачи: "GAP" — закрывает пробел (номер уже выдавался или разбирался, но не решён), "NEW" — материал, который ученик ещё не брал. minutes — твоя оценка времени на задачу для ЭТОГО ученика (целое, обязательно). comment — короткое пояснение учителю (до 200 символов). В summary — 1–2 предложения учителю: логика набора и на что обратить внимание.
