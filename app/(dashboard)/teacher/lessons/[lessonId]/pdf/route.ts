...
export async function GET(request: Request, { params }: { params: Promise<{ lessonId: string }> }) {
  ...
  const html = embedKatexAssets(
    answersOnly ? renderLessonAnswersHtml(payload.printData) : renderLessonPrintHtml(payload.printData)
  );
  ...
  const fileName = answersOnly ? `otvety-${dateLabel}.pdf` : `urok-${dateLabel}.pdf`;
  const fileNameUtf8 = encodeURIComponent(
    answersOnly ? `ответы-${groupSlug}-${dateLabel}.pdf` : `урок-${groupSlug}-${dateLabel}.pdf`
  );
