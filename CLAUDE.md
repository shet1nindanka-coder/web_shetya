...
- **AI lesson planner** — «ИИ-подбор заданий на урок»: группы (`actions/group.ts`,
  ...
  **Живой урок** (вкладка ученика `/student/lesson`): старт/конец — по расписанию `startsAt`+`durationMinutes` (`lib/lesson-status.ts`, `finishedAt` — досрочное завершение учителем). Ученик сдаёт фото по одному номеру (`LessonItemSubmission`, API `/api/student/lesson-submissions`), проверка — `lib/lesson-item-check.ts` (переиспользует ядро `solution-check.ts`, отдельный бюджет `lessonCheck*` в настройках). Итоги ставятся автоматически (`lib/lesson-live.ts`): верно с первого раза → SOLVED, верно после ошибок → PARTIAL, неверно → NOT_SOLVED, конец урока: сдавал без зачёта → NOT_SOLVED, не сдавал вовсе → SKIPPED (ленивое закрытие `lib/lesson-finalize.ts` в окне 48 ч, метка — `Lesson.status = FINISHED`); учитель может переразметить вручную. Realtime — событие `lesson-activity`.
